#!/usr/bin/env python3
"""Validate the Stage 5 published dashboard-redesign.html for a dashboard folder.

Stdlib-only (html.parser). Checks:
  - every required substring (e.g. section titles, selected type) is present
  - every local <img src="..."> resolves to a file on disk
  - every in-page <a href="#..."> resolves to an existing element id

Usage:
    python validate_dashboard_html.py <path/to/dashboard-redesign.html> \
...
def main() -> int:
    ...
    missing_text = [s for s in args.require if s not in text]

    broken_images = []
    for src in collector.images:
        if urlparse(src).scheme:  # remote asset, skip
            continue
        candidate = (args.html_path.parent / unquote(src)).resolve()
    ...
    print(
        f"HTML validation passed: {len(text):,} bytes, {len(collector.images)} images, {len(collector.ids)} ids."
    )
    return 0
